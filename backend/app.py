...
# --- CRUD PARA DISEÑOS PERSONALIZADOS ---
...
def crear_diseno():
    try:
        ...
        return jsonify({"mensaje": "¡Diseño conceptual guardado!", "id": str(resultado.inserted_id)}), 201
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
@app.route('/api/disenos/<id>', methods=['GET'])
...
@app.route('/api/disenos/<id>', methods=['DELETE'])
@jwt_required()
def eliminar_diseno(id):
    try:
        resultado = disenos_collection.delete_one({"_id": ObjectId(id)})
        if resultado.deleted_count > 0:
            return jsonify({"mensaje": "Diseño eliminado correctamente"}), 200
        else:
            return jsonify({"error": "No se encontró el diseño para eliminar"}), 404
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route('/api/disenos/<id>', methods=['PUT'])
@jwt_required()
def editar_diseno(id):
    try:
        ...
        if resultado.matched_count > 0:
            return jsonify({"mensaje": "¡Diseño actualizado con éxito!"}), 200
        else:
